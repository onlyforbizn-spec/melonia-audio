...
(function () {
  ...
  css.textContent = [
    ...
    '.mln-voice-btn{width:52px;height:52px;border-radius:50%;background:' + ORANGE + ';color:#fff;border:none;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 6px 18px rgba(233,118,75,.45);transition:transform .15s ease,box-shadow .15s ease,background .15s ease;-webkit-tap-highlight-color:transparent;flex-shrink:0;}',
    '.mln-voice-btn:hover{box-shadow:0 8px 22px rgba(233,118,75,.55);}',
    '.mln-voice-btn:active{transform:scale(.94);}',
    '.mln-voice-btn.rec{background:#d9403a;animation:mlnVoiceRing 1.4s ease-out infinite;}',
    ...
    '.mln-voice-btn.busy{background:#c9beb6;pointer-events:none;box-shadow:none;}',
